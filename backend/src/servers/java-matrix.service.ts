...
// MC version → itzg image tag selection. The image does NOT pick Java for
// you; this matrix implements the rules from docs/versions/java.md. Users
// can always override per server (servers.java_tag).
...
// GTNH is a 1.7.10 pack that also runs on modern Java via its bundled
// lwjgl3ify patches, and its release index states the highest Java each
// version supports. Ladder down to the newest tag the panel actually ships
...
export interface PickJavaTagOptions {
  maxJavaVersion?: number | null;
}

@Injectable()
export class JavaMatrixService {
  /** Exposed for callers (PacksService, SolverService) that need the raw
   *  major.minor.patch comparison this matrix already implements internally. */
  ...
  pickJavaTag(
    ...
    if (v.major === 1 && v.minor <= 16) {
      ...
      // era, and Forge < 1.18 hard-requires it.
