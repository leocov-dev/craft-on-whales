...
interface RawYamlSpec {
  ...
  volumes?: {
    extra?: { hostPath: string; containerPath: string; mode?: 'rw' | 'ro' }[];
  };
}
...
export class DockerSpecService {
  ...
  fromYaml(text: string): ParsedOverrides {
    ...
    if (!obj || typeof obj !== 'object' || Array.isArray(obj)) {
      throw new BadRequestException(
        'Invalid YAML: expected a mapping at the top level',
      );
    }
    return {
      containerName: obj.containerName || null,
      networkName: obj.network || null,
      extraPorts: (obj.ports && Array.isArray(obj.ports.extra)
        ? obj.ports.extra
        : []
      ).map((p) => ({
      ...
      extraBinds: (obj.volumes && Array.isArray(obj.volumes.extra)
        ? obj.volumes.extra
        : []
      ).map((b) => ({
        hostPath: b.hostPath,
  ...
  async validateOverrides(
    ...
    if (networkName) {
      ...
      if (!exists)
        errors.push(`Docker network "${networkName}" does not exist.`);
    }

    const previousHostPorts = new Set(
      (previousExtraPorts || []).map((p) => p.hostPort),
    );
    ...
    for (const p of extraPorts || []) {
      ...
      if (!Number.isInteger(hostPort) || hostPort < 1024 || hostPort > 65535) {
        errors.push(
          `Extra port mapping has an invalid host port: ${p.hostPort}`,
        );
        continue;
      }
      if (
        ...
        errors.push(
          `Extra port mapping has an invalid container port: ${p.containerPort}`,
        );
        continue;
      }
      if (!['tcp', 'udp'].includes(p.protocol)) {
        errors.push(
          `Extra port mapping protocol must be "tcp" or "udp", got "${p.protocol}"`,
        );
        continue;
      }
      if (seenPorts.has(hostPort)) {
        errors.push(
          `Host port ${hostPort} is used by more than one extra port mapping.`,
        );
        continue;
      }
      seenPorts.add(hostPort);
      if (previousHostPorts.has(hostPort)) continue; // unchanged from this server's own current config
      if (!(await this.ports.isPortFree(hostPort)))
        errors.push(`Host port ${hostPort} is already in use.`);
    }

    for (const b of extraBinds || []) {
      if (
        !b.hostPath ||
        typeof b.hostPath !== 'string' ||
        b.hostPath.includes('\0') ||
        !path.isAbsolute(b.hostPath)
      ) {
        errors.push(
          `Extra volume bind has an invalid host path: "${b.hostPath || ''}" — must be an absolute path.`,
        );
      }
      if (
        !b.containerPath ||
      ...
      if (b.mode && !['rw', 'ro'].includes(b.mode)) {
        errors.push(
          `Extra volume bind mode must be "rw" or "ro", got "${b.mode}"`,
        );
      }
    }
