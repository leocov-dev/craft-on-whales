import { BadRequestException, Body, Controller, ConflictException, Get, HttpCode, NotFoundException, Param, Post, Query, Req } from '@nestjs/common';
...
function sanitizePackHtml(html: unknown): string {
  return sanitizeHtml(String(html || ''), {
    allowedTags: [
      'p', 'b', 'strong', 'i', 'em', 'u', 's', 'del', 'code', 'pre', 'a', 'ul', 'ol', 'li', 'br', 'hr', 'blockquote',
      'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'img', 'span', 'div', 'table', 'thead', 'tbody', 'tr', 'th', 'td',
      'details', 'summary', 'center', 'figure', 'figcaption',
    ],
    allowedAttributes: { a: ['href', 'rel', 'target'], img: ['src', 'alt', 'title', 'width', 'height'] },
...
export class PacksController {
  ...
  async resolve(@Body() body: unknown) {
    ...
    return { ok: true, pack: await this.packs.resolvePack(platform, ref, { versionId, mcVersion }) };
  }
  ...
  async applyToServer(@Req() req: Request, @Param('id') id: string, @Body() body: unknown) {
    ...
    await this.packs.applyPack(id, resolved, { actor: req.user!.username, force });
    return { ok: true, pack: resolved, note: 'Applied — recreate/restart to install' };
  }
  ...
  async upgradePack(@Req() req: Request, @Param('id') id: string, @Body() body: unknown) {
    ...
    const taskId = this.tasks.run(`Upgrading pack on ${server.display_name}`, { serverId: server.id, actor }, async (t) => {
      ...
      try {
        return await this.upgrade.upgradePack(server.id, { versionId, skipBackup, actor, onStep: (s: string) => t.step(UPGRADE_STEP_LABELS[s] || s) });
      } catch (err) {
        const e = err as Error & { rollbackAvailable?: boolean };
        if (e.rollbackAvailable) return { ok: false, error: e.message, rollbackAvailable: true };
        throw err;
      }
    });
  ...
  @Get('packs/search')
  async search(@Query('q') q = '', @Query('platform') platform: 'modrinth' | 'curseforge' = 'modrinth') {
    const { q: qq, platform: p } = parseBody(z.object({ q: z.string().trim().min(1).max(120), platform: z.enum(['modrinth', 'curseforge']).default('modrinth') }), { q, platform });
    if (p === 'modrinth') {
      const hits = await this.modrinth.search({ query: qq, kind: 'modpack' });
      const results: PackSearchResult[] = hits.map((h) => ({ platform: p, ref: h.slug, name: h.title, iconUrl: h.iconUrl, downloads: h.downloads, description: h.description }));
      return { ok: true, results };
    }
    const hits = await this.curseforge.search({ query: qq, kind: 'modpack' });
    const results: PackSearchResult[] = hits.map((m) => ({ platform: p, ref: m.slug, name: m.name, iconUrl: m.iconUrl, downloads: m.downloads, description: m.summary }));
    return { ok: true, results };
  }
  ...
  async details(@Query('platform') platformQ?: string, @Query('ref') refQ?: string, @Query('serverId') serverIdQ?: string) {
    ...
    if (platform === 'packwiz') {
      ...
      downloads = project.downloads ?? null;
      description = sanitizePackHtml(await this.curseforge.getDescription(project.modId));
    }
    return {
      ok: true,
      pack: {
  ...
  async packMods(@Param('id') id: string) {
    ...
    return { ok: true, pack: { name: pin.projectName, version: pin.pinnedVersionName }, mods: rows };
  }
  ...
  fromPack(@Req() req: Request, @Body() body: unknown) {
    ...
    const actor = req.user!.username;
    const taskId = this.tasks.run(`Creating ${input.name} from a ${input.platform} pack`, { actor }, async (t) => {
      t.step('Resolving pack version (pinned — never "latest")');
      const resolved = await this.packs.resolvePack(input.platform, input.ref, { versionId: input.versionId });
      ...
      const server = await this.lifecycle.createServer(
        ...
        { actor, start: false, onProgress: (s: string) => t.step(s), javaTagHint: resolved.javaTag } as never
      );
      ...
      return { serverId: server.id, name: server.display_name, pack: { name: resolved.projectName, version: resolved.versionName, mcVersion: resolved.mcVersion } };
    });
    return { ok: true, taskId };
  }
}
