...
export class ServerViewModelService {
  ...
  async serverVM(
    ...
    if (
      ...
      if (info?.startedAt)
        vm.stats.uptime = formatUptime(Date.now() - Date.parse(info.startedAt));
  ...
  async packVM(serverId: string): Promise<PackViewModel | null> {
    ...
    return {
      ...
      autoRestartOnPackChange: Boolean(pack.autoRestartOnPackChange),
    };
  }
