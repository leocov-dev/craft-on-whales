...
export class EventsController {
  ...
  private async eventVM(e: { id: number; serverId: string | null; actor: string; type: string; summary: string; logExcerptPath: string | null; createdAt: string; details: Record<string, unknown> }): Promise<EventViewModel> {
    ...
    return {
      ...
      server: row ? row.displayName + (row.deletedAt ? ' (deleted)' : '') : '— panel —',
      type: e.type,
      actor: e.actor,
      ts: e.createdAt,
      summary: e.summary,
  ...
  archivedFile(@Res() res: Response, @Param('id') id: string, @Param('file') fileParam: string) {
    const file = parseBody(z.string().regex(/^[\w.,()[\] -]+$/, 'Invalid file name'), fileParam);
    const abs = path.join(this.config.dataDir, 'logs', id, 'events', file);
    if (!fs.existsSync(abs)) throw new NotFoundException('Archived log not found');
