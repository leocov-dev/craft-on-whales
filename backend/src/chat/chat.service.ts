...
import type { ChatOptions, ColorName, FormatFlag, SentChatMessage, TextComponent } from './chat.types';
...
export const FORMATS: FormatFlag[] = ['bold', 'italic', 'underlined', 'strikethrough', 'obfuscated'];
...
export class ChatService {
  ...
  normalizeTarget(target: unknown): string {
    ...
    throw new BadRequestException('Invalid recipient — pick Everyone or a valid player name');
  }
  ...
  async sendChat(serverId: string, opts: ChatOptions = {}): Promise<SentChatMessage & { actor: string; ts: string }> {
    const text = String(opts.text || '')
      .replace(/[\r\n]+/g, ' ')
      .trim();
    if (!text) throw new BadRequestException('Message text is required');
    if (text.length > 512) throw new BadRequestException('Message is too long (512 chars max)');
    ...
    if (mode === 'say') {
      ...
      cmd = ['tellraw', target, JSON.stringify(this.buildComponent({ ...opts, text }))];
    }

    const out = cleanText(await this.containers.execCapture(serverId, ['rcon-cli', ...cmd]));
    if (out.trim() && /Unknown or incomplete|Incorrect argument|Expected|No player was found|<--\[HERE\]/i.test(out)) {
      throw new HttpException(`The server rejected the message: ${out.split('\n')[0]}`, 502);
    }
