...
export class MojangService {
   ...
   */
  async listVersions({ includeSnapshots = false, includeAll = false, limit = 200 }: { includeSnapshots?: boolean; includeAll?: boolean; limit?: number } = {}): Promise<MojangVersionEntry[]> {
    const manifest = await this.getVersionManifest();
    return manifest.versions.filter((v) => (includeAll ? true : v.type === 'release' || (includeSnapshots && v.type === 'snapshot'))).slice(0, limit);
  }
}
