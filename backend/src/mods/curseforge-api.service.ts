import { Injectable, NotFoundException, ForbiddenException, HttpException, BadGatewayException, PreconditionFailedException } from '@nestjs/common';
import { ApiCacheService } from './api-cache.service';
import { ApiKeysService } from '../api-keys/api-keys.service';
import type { CurseforgeMod, CurseforgeFile, CurseforgeResolved } from './mods.types';
...
/** CurseForge API client (requires the user's API key from the encrypted store). Docs: https://docs.curseforge.com — Minecraft gameId = 432. */
@Injectable()
export class CurseforgeApiService {
  ...
  private async cfFetch<T = unknown>(pathname: string, { search, ttlMs = 10 * 60 * 1000, method = 'GET', body }: CfFetchOptions = {}): Promise<T> {
    ...
    if (res.status === 429) {
      ...
      throw new HttpException('CurseForge rate limit hit — try again in a minute', 429);
    }
    if (res.status === 403) throw new ForbiddenException('CurseForge rejected the API key — re-check it in Settings');
    if (res.status === 404) throw new NotFoundException('Not found on CurseForge');
    if (!res.ok) throw new BadGatewayException(`CurseForge answered HTTP ${res.status}`);
    const data = (await res.json()) as T;
    if (method === 'GET') this.cache.set(cacheKey, data);
    return data;
  }

  async search({ query = '', kind = 'mod', mcVersion, loader, limit = 20, index = 0 }: CurseforgeSearchParams): Promise<CurseforgeMod[]> {
    ...
    const data = await this.cfFetch<{ data: RawCfMod[] }>('/mods/search', { search: params, ttlMs: 5 * 60 * 1000 });
    return data.data.map((m) => this.normalizeMod(m));
  }
  ...
  async getModBySlug(slug: string, { classId = CLASS_MODPACKS }: { classId?: number } = {}): Promise<CurseforgeMod | null> {
  ...
  async getFiles(modId: number, { mcVersion, loader, pageSize = 50 }: { mcVersion?: string; loader?: string; pageSize?: number } = {}): Promise<CurseforgeFile[]> {
    const params: Record<string, string | number> = { pageSize };
    if (mcVersion) params.gameVersion = mcVersion;
    if (loader) params.modLoaderType = this.loaderTypeId(loader);
    const data = await this.cfFetch<{ data: RawCfFile[] }>(`/mods/${modId}/files`, { search: params, ttlMs: 10 * 60 * 1000 });
    return data.data.map((f) => this.normalizeFile(f));
  }

  async getFile(modId: number, fileId: number): Promise<CurseforgeFile> {
    const data = await this.cfFetch<{ data: RawCfFile }>(`/mods/${modId}/files/${fileId}`, { ttlMs: 60 * 60 * 1000 });
  ...
  async getDescription(modId: number): Promise<string> {
    const data = await this.cfFetch<{ data: unknown }>(`/mods/${modId}/description`, { ttlMs: 30 * 60 * 1000 });
   ...
   */
  async resolveUrl(input: string): Promise<CurseforgeResolved> {
    ...
    const mod = /^\d+$/.test(slug) ? await this.getMod(Number(slug)) : (await this.getModBySlug(slug, { classId })) || (await this.getModBySlug(slug, { classId: CLASS_MODS }));
    if (!mod) throw new NotFoundException(`CurseForge project "${slug}" not found`);
    return { ...mod, fileId };
  }

  private normalizeMod(m: RawCfMod): CurseforgeMod {
    return {
      modId: m.id,
      slug: m.slug,
      name: m.name,
      ...
      classId: m.classId,
      latestFiles: (m.latestFiles || []).map((f) => this.normalizeFile(f)),
    };
  }

  private readonly RELEASE_TYPES: Record<number, 'release' | 'beta' | 'alpha'> = { 1: 'release', 2: 'beta', 3: 'alpha' };

  private normalizeFile(f: RawCfFile): CurseforgeFile {
    return {
      fileId: f.id,
      name: f.displayName,
      fileName: f.fileName,
      ...
      releaseType: (f.releaseType !== undefined && this.RELEASE_TYPES[f.releaseType]) || 'release',
      fileDate: f.fileDate,
  ...
  private readonly LOADER_TYPE_IDS: Record<string, number> = { forge: 1, fabric: 4, quilt: 5, neoforge: 6 };
