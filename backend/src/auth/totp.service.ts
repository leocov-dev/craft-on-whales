...
export class TotpService {
  readonly stepSeconds = STEP_SECONDS;
  ...
  generateSecret(): string {
    return this.base32Encode(crypto.randomBytes(SECRET_BYTES));
  }
  ...
  private hotp(secretBytes: Buffer, counter: number): string {
  ...
  private currentStep(atMs: number = Date.now()): number {
  ...
  verify(
    ...
    const step = this.currentStep(atMs);
  ...
  codeAt(secret: string, atMs: number = Date.now()): string {
    return this.hotp(this.base32Decode(secret), this.currentStep(atMs));
  }

  /** `n` random backup codes, formatted xxxx-xxxx for readability. */
  generateBackupCodes(n: number = BACKUP_CODE_COUNT): string[] {
    const codes: string[] = [];
    for (let i = 0; i < n; i++) {
