...
export function assertUuid(uuid: string | null | undefined): string {
  const u = String(uuid || '').toLowerCase();
  if (!UUID_RE.test(u)) throw new BadRequestException('Invalid player UUID');
  return u;
}
...
export function assertItemId(item: string | null | undefined): string {
...
export function textComponentToString(value: any): string | null {
...
export function normalizeItem(raw: any): NormalizedItem | null {
...
export const NESTED_MAX_PATH = 10; // path segments from the item root
export const NESTED_KEY_RE = /^[A-Za-z0-9_:./ -]{1,80}$/;
...
export function normalizeItemDeep(raw: any): NormalizedItem | null {
