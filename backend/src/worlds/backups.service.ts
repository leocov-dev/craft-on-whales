...
export class BackupsService {
  ...
  async createBackup(serverId: string, { reason = 'manual', actor = 'system', note = '', task = null }: CreateBackupOptions = {}) {
    ...
    const doArchive = async () => {
      ...
      await this.zipDirectory(this.pathGuard.dataPath('servers', serverId), absPath, {
    ...
    if (running) {
      ...
      await this.saveLock.withSaveLock(serverId, async () => {
        ...
        const paused = await this.containers
          ...
          .then(() => true)
          .catch((err: Error) => {
            console.warn(`[backup] save-off failed for ${serverId}: ${err.message} — archive may be slightly inconsistent`);
            return false;
          });
        inconsistent = !paused;
  ...
  async restoreBackup(serverId: string, backupId: string, { actor = 'system', skipSafety = false, task = null }: RestoreBackupOptions = {}): Promise<{ ok: true }> {
    ...
    if (!zipStat) throw new NotFoundException(`Backup archive is missing on disk: ${backup.filename}`);
    const { free } = await this.indexer.diskFree();
    if (free < zipStat.size * 2) {
      throw new HttpException(`Not enough disk space to restore (~${((zipStat.size * 2) / 1024 ** 3).toFixed(1)} GB needed)`, 507);
    }
    ...
    if (info.exists && ['running', 'starting', 'unhealthy'].includes(info.status)) {
      throw new ConflictException('The server did not stop — restore aborted to avoid corrupting the live world. Stop it manually and retry.');
    }

    if (!skipSafety) {
      if (task) task.step('Creating safety backup');
      await this.createBackup(serverId, { reason: 'manual', actor, note: `Safety backup before restoring ${backup.filename}`, task: null });
    }
    ...
    await this.archive.extractZip(this.pathGuard.dataPath(backup.relPath), serverDir);

    this.events.recordEvent({ serverId, actor, type: 'backup-restored', summary: `Restored backup ${backup.filename}` });
    this.indexer.scan().catch(() => {});
    return { ok: true };
  }
  ...
  private zipDirectory(sourceDir: string, outFile: string, { onProgress = null }: { onProgress?: ((processedBytes: number) => void) | null } = {}): Promise<void> {
