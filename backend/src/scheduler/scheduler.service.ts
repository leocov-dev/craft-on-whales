...
import type { CreateScheduleInput, ScheduleViewModel } from '../../../shared/types/schedules';
...
export class SchedulerService implements OnModuleInit {
  ...
  private async runTask(job: ScheduleRow): Promise<void> {
    const payload: Record<string, unknown> = JSON.parse(job.payloadJson || '{}');
    ...
    switch (job.taskType) {
      ...
      case 'start':
        ...
        break;
      case 'backup':
        await this.backups.createBackup(job.serverId!, { reason: 'scheduled', actor });
        break;
      case 'rcon': {
  ...
  private async schedule(job: ScheduleRow): Promise<void> {
    ...
    try {
      ...
      console.error(`[scheduler] invalid cron "${job.cron}" for ${job.id}: ${err instanceof Error ? err.message : String(err)}`);
    }
  }
