import { BadRequestException, Controller, Delete, ForbiddenException, Get, NotFoundException, Param, Post, Req, Res, UploadedFile, UseInterceptors } from '@nestjs/common';
...
import { BlueprintsLibraryService, type DecoratedBlueprint } from './blueprints-library.service';
 ...
 */
function publicBlueprint(b: DecoratedBlueprint | null): BlueprintViewModel | null {
  if (!b) return null;
  return {
    id: b.id,
    name: b.name,
...
export class BlueprintsController {
  ...
  async export(@Req() req: Request) {
    const input = parseBody(
      z.object({ serverId: z.string().trim().min(1).max(40), includeConfig: z.coerce.boolean().optional(), embedFiles: z.coerce.boolean().optional(), includeWorld: z.coerce.boolean().optional() }),
      req.body
    );
  ...
  @UseInterceptors(
    FileInterceptor('file', {
      storage: multer.diskStorage({
        ...
        filename: (_req, _file, cb) => cb(null, `bpup-${nanoid(10)}.mcserver.zip`),
      }),
      limits: { fileSize: 8 * 1024 ** 3 },
    })
  )
  async importPreview(@UploadedFile() file: Express.Multer.File | undefined, @Req() req: Request) {
    if (file) {
      let preview;
      try {
  ...
  async import(@Req() req: Request) {
    const input = parseBody(
      z
        .object({ blueprintId: z.string().trim().min(1).max(40).optional(), uploadToken: uploadTokenSchema.optional(), overrides: overridesSchema.optional() })
        .refine((v) => Boolean(v.blueprintId) !== Boolean(v.uploadToken), { message: 'Provide exactly one of blueprintId or uploadToken' }),
      req.body
    );
    ...
    if (input.uploadToken) {
      ...
      if (!fs.existsSync(zipRef)) throw new NotFoundException('Uploaded blueprint expired — upload it again');
    }
    if (input.overrides) requireAdminForOverrides(req, input.overrides);
    const { server, report } = await this.importService.importBlueprint(zipRef as string, input.overrides || {}, { actor: req.user!.username });
  ...
  async clone(@Req() req: Request) {
    const input = parseBody(z.object({ serverId: z.string().trim().min(1).max(40), includeWorld: z.coerce.boolean().optional() }), req.body);
    const { server, report, blueprint } = await this.importService.cloneServer(input.serverId, { includeWorld: input.includeWorld, actor: req.user!.username });
    return {
      ok: true,
  ...
  async remove(@Param('id') id: string, @Req() req: Request) {
    return { ok: true, ...(await this.library.deleteBlueprint(id, { actor: req.user!.username })) };
  }
}
