...
| `SESSION_SECRET`                                                            | auto-generated             | Signs session cookies. Also derives the at-rest encryption key as a deprecated fallback when `SECRET_KEY` is unset. Auto-created and persisted if unset.                                                                                                                                                                                                                                                                                 |
| `SECRET_KEY`                                                                | —                          | Dedicated at-rest encryption key for stored secrets (API keys, RCON passwords, 2FA secrets, webhook URLs) — 32 bytes, hex or base64 (`openssl rand -hex 32`). Recommended; if unset, falls back to a `SESSION_SECRET`-derived key with a boot warning. Never generated or written to disk by the panel — you must set it yourself, and **losing it after adopting it is not recoverable**: every stored credential has to be re-entered. |
| `TRUST_PROXY` / `COOKIE_SECURE`                                             | —                          | Set when behind a TLS-terminating reverse proxy, so `req.ip` (rate-limiting) and `Secure` cookies work. `TRUST_PROXY` takes a hop count (e.g. `1`) or a comma-list of trusted proxy IPs/CIDRs — a bare `true` is rejected as spoofable.                                                                                                                                                                                                  |
| `COOKIE_SAMESITE`                                                           | `lax`                      | Session cookie's `SameSite` policy: `lax`, `strict`, or `none`. `none` also requires `COOKIE_SECURE=true` (or `auto` behind a reverse proxy) — browsers reject a non-`Secure` `SameSite=None` cookie.                                                                                                                                                                                                                                    |
...
API keys, RCON passwords, 2FA secrets, and Discord webhook URLs are encrypted with AES-256-GCM,
using `SECRET_KEY` if you've set one, or a key derived from `SESSION_SECRET` otherwise (deprecated
fallback — set `SECRET_KEY` so encryption doesn't share a key with session-cookie signing).
**`SECRET_KEY` is never generated or written to disk by the panel** — you supply it, and if you lose
it after adopting it, every stored credential is unrecoverable and must be re-entered; there's no
recovery mechanism. Blueprints never contain secrets. The panel refuses to set footgun env vars
(`REMOVE_OLD_MODS`, `LOAD_ENV_FROM_*`).
